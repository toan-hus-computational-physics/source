...
/* erfc from glibc source. Simplified for GCMC since argument is always > 0 */
/*
 * ====================================================
 ...
 * #include <math_private.h> 
   the latest in the directory sysdeps/generic/math_private.h
   of the glibc source
...
double erfc_positive_arg(double x)
{
	int32_t hx,ix;
	double R,S,P,Q,s,y,z,r;
	GET_HIGH_WORD(hx,x);
...
// 	    }
	...
	if (ix < 0x403c0000) {		/* |x|<28 */
	    ...
	    SET_LOW_WORD(z,0);
#ifdef __SSE2__
	    __m128d t = (__m128d){ -z*z-0.5625, (z-x)*(z+x)+R/S }, se;
	    se = sse_exp(t);
	    return ((double *)(&se))[0]*((double *)(&se))[1]/x;
#else
